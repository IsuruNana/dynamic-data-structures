...
{
	if(items->itemsCount < items->arrayLength) //The array is big enough
	{	
		items->itemsArray[items->itemsCount] = value;
		items->itemsCount++;
	}
	
	else if(items->itemsCount == items->arrayLength) // Array needs to expand
	{
...
void *removeDA(DA *items)
{
    ...
    items->itemsCount--;
    void *returnValue = items->itemsArray[items->itemsCount];
    if((float) items->itemsCount < (float)  ((float) items->arrayLength / (float) 4)) //Shrink if necassary
    {
        int newSize = items->arrayLength/2;
	items->itemsArray = realloc(items->itemsArray, sizeof(void*) * newSize);
	items->arrayLength = newSize;
